...
public class ScanSceneController : MonoBehaviour
{
	...
	public XElement GetCurrentItemInfo(){
		Logger.Log ("GetCurrentItemInfo " + currentTrackableObject.name); 
		return Xml.GetChildByAttribute (itemInfos, "id", currentTrackableObject.name);
	}

	public XElement GetItemInfo(string name){
		return Xml.GetChildByAttribute (itemInfos, "id", name);
	}
	...
	//if forceOverride prev state = prev state return function;
	public void SetState (string name, Hashtable args = null, bool forceOverride = true)
	{
		if (state != null && state.name == name && !forceOverride) {
			return;
		}
		if (state != null) {
			state.OnExit ();
...
//			}
	...
	protected GameObject GetAsset(string name){
		if (loadedAssets.ContainsKey (name))
			return loadedAssets [name] as GameObject;
		if (AssetManager.assets.ContainsKey (name))
