...
public class BloodController : MonoBehaviour, ITrackableController {
	...
	public void ShowArrows(bool b = true){
		for (int i = 0; i < arrows.Count; i++) {
			arrows[i].SetActive(b);
		}
	}
	...
	void Update () {
		...
		if (spawning) {
			for (int i = 0; i < arrows.Count; i++) {
				arrows[i].GetComponent<MeshRenderer>().material.SetColor("_Color", new Color(1,1,1,Mathf.Sin(arrowTimer * 6.15f)));
			}
		}
	}
	...
	virtual public void OnTrackingFound ()
	{
		...
		for (int i = 0; i < renderers.Length; i++) {
			if(renderers[i].gameObject.name.Contains("KnowledgePoint"))
