...
//				PlayVideo (videoPath);
		...
		if (disableChildren) {
			this.gameObject.EnableChildren ();
		}
...
//			}
		if (disableChildren) {
			this.gameObject.EnableChildren (false);
		}
		if (!ScanSceneController.instant.exited && !ScanSceneController.instant.state.isFullScreen)
